...
def getitem(event, context):
  ...
  elif "THXTool" in js_event_body["result"]["parameters"]: # said_THX
  ...
  else: # Catch SUSHI Req.
    param_channel = js_event_body["originalRequest"]["data"]["event"]["channel"]
    param_ts = js_event_body["originalRequest"]["data"]["event"]["ts"]
    fuE = {
      "followupEvent": {
        "name": "incremented",
        "data": {
          "post_channel":param_channel,
          "post_ts":param_ts
        }
      }
    }
